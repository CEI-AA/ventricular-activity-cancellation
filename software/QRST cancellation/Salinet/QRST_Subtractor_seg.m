...
for n=1:length (Intracardiac_signals_raw(1,:)) 
    ...
    nwins_pattern=floor(length(Final_yw_intra(:,n))/win_track_pattern)
    newbegin = 0; newend = 0;
    
     for n_patterns=1:nwins_pattern
      
       signal_seg=Final_yw_intra(((n_patterns-1)*win_track_pattern+1):n_patterns*win_track_pattern,n);
       ...
       if Modelo_Subtractor == 1
        [pattern,newseg,Onr,Tendvr,newbegin,newend] = Prot_Beat_Seg(begin_seg(n_patterns),end_seg(n_patterns),signal_seg,n);
       else [pattern,newseg,Onr,Tendvr,newbegin,newend] = Prot_Beat_Seg2(begin_seg(n_patterns),end_seg(n_patterns),signal_seg,n);
       end
        signal_seg = newseg;
        ...
        for n_QRST = 1:length(Onr)
            ...
            if n_QRST == length(Onr)
                ...
                signal_seg(Onr(n_QRST)-delaypeak:min(Onr(n_QRST)-delaypeak+lp-1,length(signal_seg))) = signal_seg(Onr(n_QRST)-delaypeak:min(Onr(n_QRST)-delaypeak+lp-1,length(signal_seg))) - pattern(1:min(length(signal_seg),Onr(n_QRST)-delaypeak+lp-1)-Onr(n_QRST)+delaypeak+1);
            else signal_seg(Onr(n_QRST)-delaypeak:Onr(n_QRST)-delaypeak+lp-1) = signal_seg(Onr(n_QRST)-delaypeak:Onr(n_QRST)-delaypeak+lp-1) - pattern;
...
Final_yw_intra_sub_final = [];
 for icat = 1:n_patterns
     a = Final_yw_intra2_cell{1,icat}{1};
