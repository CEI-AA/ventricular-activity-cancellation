function[pattern,segment,Onr,Tendvr,begin_seg,end_seg] = Prot_Beat_Seg_by_beat(begin_seg,end_seg,segment,n_patterns)
...
if On_seg(end) > R_seg(end)
    Tendv(Tend_seg(end)) = On(On_seg(end))-1;
    end_seg = Tendv(Tend_seg(end));
    On_seg = find(On >= begin_seg & On <= end_seg);
end
...
segment=Final_yw_intra(begin_seg:end_seg);
